...
// private log messages are not logged unless they have been explicitly enabled by calling AMLSetLogLevel().
...
static bool AdjustAndVerifyLogLevel(int* level)
{
    ...
    if (AML_LOG_PRIVATE_DATA & localLevel)
    {
        ...
        localLevel &= ~AML_LOG_PRIVATE_DATA;
    }
...
void AMLLogBuffer(int level, const char* tag, const uint8_t* buffer, size_t bufferSize)
{
    ...
    for (i = 0; i < bufferSize; i++)
    {
        ...
        if (((i + 1) % 16) == 0)
        {
            AMLLogv(level, tag, "%s", lineBuffer);
            memset(lineBuffer, 0, sizeof lineBuffer);
            lineIndex = 0;
        }
    }
